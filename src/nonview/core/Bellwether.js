...
export default class Bellwether {
    static getStatsForElection(election, entID) {
        ...
        const isSameWinner = resultEnt.partyToVotes.winningPartyID === resultLK.partyToVotes.winningPartyID;
        const error = Object.entries(resultLK.partyToVotes.partyToPVotesSorted).reduce(
            function(error ,[partyID, pVotesLK]) {
    ...
    static getStats(elections, electionDisplay) {
        ...
        const previousElectionsOfSameType = Election.getPreviousElectionsOfSameType(elections, electionDisplay);

        const n = previousElectionsOfSameType.length;
        return previousElectionsOfSameType.reduce(
            function({n, nSame, error}, election) {
                const {isSameWinner, error: errorElection} = Bellwether.getStatsForElection(election, entID);
                return {
                    n: n ,
