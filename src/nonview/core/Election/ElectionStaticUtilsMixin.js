...
const ElectionStaticUtilsMixin = {
     getProvinceID(edID) {
       ...
       getPreviousElectionsOfSameType(elections, currentElection) {
        return ElectionStaticUtilsMixin.getPreviousElections(elections, currentElection).filter(
          function (election) {
       ...
       getPenultimateElectionOfSameType(elections, currentElection) {
        const previousElections = ElectionStaticUtilsMixin.getPreviousElectionsOfSameType(
          elections,
          currentElection
        );
        return previousElections[previousElections.length - 1];
