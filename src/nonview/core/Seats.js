...
export default class Seats {
    ...
    get regionToPartyToSeats() {
        let idx = this.election.edResultList.reduce(
            function(idx, result) {
                ...
                const partyToSeats = Seats.getGenericPartyToSeats(result, nSeatsAll, 1, 0.05);
        ...
        idx['LK'] = Seats.getGenericPartyToSeats(this.election.resultLK, this.regionToSeats['LK'],0 , 0);
        return idx;
    }
    ...
    get partyToSeatsMain () {
        ...
        return Object.fromEntries(Object.entries(partyToSeats).filter(
            ([partyID, seats]) => seats > Seats.MIN_SEATS_FOR_DISPLAY && partyID !== Party.ERROR.id
        ));
    ...
    static getGenericPartyToSeats(result, nSeatsAll, nSeatsBonus, pLimit) {
        ...
        const partyToSeatsF = Object.fromEntries(
            Object.entries(filteredPartyToVotes).map(
                ([partyID, votes]) => [partyID, nSeatsNonBonus * votes / filteredTotalVotes]
            ));

        let partyToSeats = Object.fromEntries(
            Object.entries(partyToSeatsF).map(
                ([partyID, seatsF]) => [partyID, parseInt(seatsF)]
            ));
        const partyToSeatsR = Object.fromEntries(
            Object.entries(partyToSeatsF).map(
                ([partyID, seatsF]) => [partyID, seatsF - partyToSeats[partyID]]
            ));

        const nSeatsI = MathX.sum(Object.values(partyToSeats));
        const nSeatsR = nSeatsNonBonus - nSeatsI;

        partyToSeats = Object.entries(partyToSeatsR).sort(
            ([partyID1, seatsR1], [partyID2, seatsR2]) => seatsR2 - seatsR1
        ).slice(0, nSeatsR).reduce(
            (acc, [partyID, seatsR]) => {
