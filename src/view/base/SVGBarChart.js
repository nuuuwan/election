...
function SVGBarChartSegment ({data, formatValue, pHeight}) {
    ...
    return (
        <g>
            <rect key={label} x={data.cumX} y="0" width={data.x} height={pHeight} fill={color} />
            
            <text x={data.cumX + data.x / 2} y={pHeight * 0.5}      
                textAnchor="middle"
...
export default function SVGBarChart({dataList, formatValue,  sx={}}) {
    ...
    let cumX = 0;
    const extendedDataList = dataList.map(
        function (data) {
    ...
    return (
        <svg viewBox={viewBox} width={width} height={height} fontFamily={THEME_DATA.typography.fontFamily}>
            {extendedDataList.map(
                function(data) {
                    return <SVGBarChartSegment key={data.label} data={data} formatValue={formatValue} pHeight={pHeight} />;
                }
            )}
        </svg>
