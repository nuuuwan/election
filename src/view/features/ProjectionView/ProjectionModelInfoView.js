    ...
    <>
      ...
      <CustomAlert severity="warning">
        <Typography variant="body1">
          {Translate("Our model needs at least %1 results to make a projection.", [ProjectionModelInfo.MIN_N_RESULTS_DISPLAY])}
...
function ProjectModelInfoViewIsTooMuchError({projectionModelInfo}) {
  ...
  return (
    <>
      ...
      <CustomAlert severity="warning">
        <Typography variant="body1">
          {Translate("Model Error is currently %1 of projected votes.", [Format.percent(projectionModelInfo.error)])}
          {Translate(" We will show our Model Projection once the Error is below %1.", [Format.percent(ProjectionModelInfo.MIN_P_ERROR)])}
        </Typography>
      </CustomAlert>
    </>
  )
...
export default function ProjectModelInfoView({children}) {
  ...
  const projectionModelInfo = new ProjectionModelInfo(electionProjected, nResultsDisplay);
  ...
  if (projectionModelInfo.isTooMuchError) {
    return <ProjectModelInfoViewIsTooMuchError projectionModelInfo={projectionModelInfo} />;
  }
