...
function SVGSeatCircle({i, n, x, y, partyID, seats}) {
    ...
    return (
        <g>
            <circle cx={cx} cy={cy} r={R_CIRCLE* 0.6} fill={color} stroke={stroke} strokeDasharray={strokeDasharray} strokeWidth={R_CIRCLE / 10} />
            <text x={cx} y={cy} textAnchor="middle" dominantBaseline="middle" fill={colorText} fontSize={R_CIRCLE * 0.8}>
                {seats}
            </text>
        </g>
    )
...
function SVGSeatCircles({x, y, partyToSeats, label}) {
    ...
    return <g>
        {Object.entries(partyToSeats).reverse().map(
            function([partyID, seats], i) {
                
                return <SVGSeatCircle key={i} i={i} n={n} x={x} y={y} partyID={partyID} seats={seats}/>

        })}
        <text x={x} y={y} textAnchor="middle" dominantBaseline="middle" fill="gray" fontSize={0.15}>
            {label}
        </text>
    </g>;
}
...
export default function SeatsHexMap() {
    ...
    const customOverlayRenderer = function({x, y, entID}) {
        ...
        return <SVGSeatCircles x={x} y={y} partyToSeats={partyToSeats} label={edIdx[entID].name} />
    }
    ...
    >
      <SVGHexMap mapData={mapData} customOverlayRenderer={customOverlayRenderer}/>
      <SVGSeatCircles x={5} y={0} partyToSeats={seats.lkPartyToSeats} label={"National List"} />;
        </svg>
    );
}