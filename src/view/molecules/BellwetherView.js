...
function PerfectBellwether({n }) {
    return (
        <Box>
            ...
            <Typography variant="caption">Results have matched Final National Result in all {n} previous Presidential Elections.</Typography>
        </Box>
    );
}
...
function PercentageBellwether({error}) {
    return (
        <Box color={STYLE.COLOR.LIGHT}>
            <Typography variant="caption">Historically, Party Vote percentages vary from the National Result by {Format.percentError(error)}, on average.</Typography>
        </Box>
    );
}
